...
export default function LineEditor({ line, onUpdate, onPlayEffect, onFileUpload }: LineEditorProps) {
  if (!line) {
    return (
    ...
    <div className="space-y-5 p-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="rounded-2xl border border-[#00f0ff]/10 bg-[#0d0d15] p-5">
          ...
          <input
            ...
            className="mt-3 w-full rounded-xl border border-dashed border-[#00f0ff]/20 bg-black/60 px-4 py-3 text-xs text-white/50 file:mr-3 file:rounded-full file:border-0 file:bg-[#00f0ff]/20 file:px-3 file:py-1.5 file:text-[10px] file:font-bold file:text-[#00f0ff]"
          />
          <p className="mt-2 text-[10px] text-white/30 font-mono">
            {line.audioName ?? "Sem arquivo: usa risada sintética ou beep"}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className="rounded-full bg-gradient-to-r from-[#00f0ff] to-[#7c3aed] px-5 py-2.5 text-xs font-bold text-black transition hover:opacity-90"
        >
          Testar efeito
        </button>
      </div>
