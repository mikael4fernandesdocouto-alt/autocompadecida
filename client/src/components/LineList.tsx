              ...
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex items-center gap-1">
                  <span className={`rounded-full px-2 py-0.5 text-[9px] font-mono ${
                    line.audioBlob ? "bg-[#00ff88]/10 text-[#00ff88] border border-[#00ff88]/20" : "bg-white/5 text-white/30"
                  }`}>
                    {line.audioBlob ? "áudio" : "sem som"}
                  </span>
                  {canDelete && (
                    <button
                      onClick={e => { e.stopPropagation(); onDelete(line.id); }}
                      className="p-1 rounded opacity-0 group-hover:opacity-100 hover:bg-red-500/20 text-red-400/50 hover:text-red-400 transition-all"
                    >
