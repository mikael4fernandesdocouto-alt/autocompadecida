    ...
    <div className="space-y-4 p-5 pt-0">
      ...
      <div className="rounded-2xl border border-[#00f0ff]/10 bg-[#0d0d15] p-4">
        ...
        <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/5">
          <div className="h-full rounded-full bg-gradient-to-r from-[#00f0ff] to-[#7c3aed] transition-all" style={{ width: `${Math.min(100, bestPreview)}%` }} />
        </div>
      ...
      <div className="rounded-2xl border border-[#00f0ff]/10 bg-[#0d0d15] p-4">
        ...
        {lastMatch ? (
          <div className="mt-2 rounded-xl border border-[#00ff88]/30 bg-[#00ff88]/5 p-3">
            ...
            <p className="mt-1.5 text-xs leading-relaxed text-white/70">{lastMatch.line.text}</p>
            <p className="mt-1 text-[10px] text-white/40 font-mono">Efeito: {lastMatch.line.effectName}</p>
          </div>
        ) : (
          <p className="mt-2 text-xs text-white/30">Nenhuma fala reconhecida ainda.</p>
        )}
      </div>
    </div>
  );
