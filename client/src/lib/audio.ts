...
export function playAudioBlob(blob: Blob, onError?: () => void): Promise<void> {
  return new Promise((resolve, reject) => {
    const url = URL.createObjectURL(blob);
    const audio = new Audio(url);
    audio.onended = () => { URL.revokeObjectURL(url); resolve(); };
    audio.onerror = () => { URL.revokeObjectURL(url); onError?.(); reject(); };
    audio.play().catch(() => { URL.revokeObjectURL(url); onError?.(); reject(); });
  });
}
